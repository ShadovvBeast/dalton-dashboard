...
const Widget = observer(props => {
  ...
  return (
    <Draggable
      handle="strong"
      defaultPosition={props.widget.location}
      onStop={onDragStop}
    >
      <div>
        <ResizableBox
        ...
        >
          ...
          <div>
            ...
            <Graph
              ...
              width={props.widget.size.width}
              height={props.widget.size.height}
              sensor_data={store.sensor_data
                .filter(sensor => {
                  for (const sensor_id of props.widget.sensor_ids) {
                    if (sensor.sensor_id === sensor_id) return true;
                  }
                  return false;
                })}
                // .map(sensor => sensor.data)}
            />
            {/* )} */}
          </div>
        </ResizableBox>
